...
class M_customer extends MY_Model
{
    ...
    function search($d)
    {
        ...
        $id = isset($d['customer_id']) ? $d['customer_id'] : 0;
        ...
        $r = $this->db->query(
            "SELECT M_CustomerID customer_id, M_CustomerName customer_name,
                ...
                M_CustomerJoinDate customer_join_date
                ...
                AND (`M_CustomerName` LIKE ? OR M_CustomerCode LIKE ? OR M_CustomerAddress LIKE ? OR M_CustomerPhones LIKE ?)
            ...
            [
                $id, $id, $id,
                $d['search'], $d['search'], $d['search'], $d['search'], $d['province'], $d['province'], $d['province'], $d['city'], $d['city'], $d['city'], $d['type'], $d['type'], $d['type']
            ]
        );
        if ($r) {
            $r = $r->result_array();
    ...
    function save($d, $id = 0)
    {
        $r = $this->db->query("CALL sp_master_customer_save(?,?,?,?,?)", [
            $id, json_encode([
                ...
                'profiles' => $d['customer_profiles']
            ]), isset($d['bdata']) ? $d['bdata'] : '[]', isset($d['addresses']) ? $d['addresses'] : '[]', $d['user_id']
        ])->row();
        ...
        // $r = $this->db->set('M_CustomerName', $d['search'])
